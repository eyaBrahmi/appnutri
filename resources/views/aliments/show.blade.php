...
<!-- Modal -->
<div class="modal fade" id="show_aliment" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="container" id="aliment">
            ...
            <form method="POST" action="{{ URL::to('/ajouter/checked')}}">
                {{ csrf_field() }}
                {{-- <input type="hidden" name="aliment_id" value="{{ $aliment->id}}"> --}}
              @foreach ($aliment as $aliment )
              
              <input type="checkbox"  > {{$aliment->nom_aliment}}</h1></br>
              @endforeach
